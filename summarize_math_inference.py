...
def scan_files(root: str, split: Optional[str]) -> List[str]:
    ...
    # stable order by step then path
...
class StepAgg:
    def __init__(self, step: int):
        ...
        # problem -> per-pass correctness (OR across samples)
        ...
        # reconsider markers (optional: counts overall rate)
        self.reconsider_rate_p1_numer = 0
        self.reconsider_rate_p2_numer = 0

        # example set for denominator
    ...
    # Render a one-line summary row (table)
    def row(self) -> str:
    ...
    # Render per-step footer with richer diagnostics
    def footer(self) -> str:
        def fmt_counter(cnt: Counter, den: int) -> str:
            ...
            parts = []
            for k in keys:
                parts.append(f"{k}={pct(cnt.get(k,0), den)}")
            return ", ".join(parts)
        ...
        if self.n_samp_p1:
            rec1 = pct(self.reconsider_rate_p1_numer, self.n_samp_p1)
            lines.append(f"   • p1 reconsider-markers rate: {rec1}")
        if self.n_samp_p2:
            rec2 = pct(self.reconsider_rate_p2_numer, self.n_samp_p2)
            lines.append(f"   • p2 reconsider-markers rate: {rec2}")

        # Optional: token lengths (only if present often)
...
def main():
    ...
    # Aggregators per step
    steps: Dict[int, StepAgg] = {}

    # Optional per-example export rows
    ex_rows: List[Tuple[int, str, int, int]] = []  # (step, problem, p1_correct, p2_correct)
    ...
    # Finalize + print table
    for step, agg in steps.items():
        agg.finalize()
