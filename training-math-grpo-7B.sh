...
export HF_TOKEN="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"
...
NUM_TRAINING=$(( NUM_TOTAL - 2 )) #Reserve 2 GPU for VLLM
# ----------------------------
...
export WANDB__SERVICE_WAIT=120   # or even 300
# -----------------------------------
# Launch vLLM + trainer in one srun
# -----------------------------------
srun --gres=gpu:8 --cpus-per-task=64 bash -c '
set -euo pipefail
export VLLM_MP_START_METHOD=spawn
############################
# 1) vLLM on GPU-0
############################
...
############################
# 2) Training on GPU 2-7
############################
export CUDA_VISIBLE_DEVICES=$(echo "'"$ALL_GPUS"'" | cut -d"," -f3-)
...
accelerate launch \
  ...
  --resume_from_checkpoint /n/fs/similarity/open-r1/data/Qwen2.5-7B-Open-R1-GRPO-math-7b/checkpoint-500 \
  --seed 42 \
  > "'"$TRAINING_LOG"'" 2>&1
